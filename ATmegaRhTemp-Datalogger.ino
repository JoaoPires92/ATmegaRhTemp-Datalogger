...
String DeviceModel = "ATmega RHTemp-04"; //Tipo de aparelho --> nome nao deve ser mais longo que 18 caracteres
String DeviceSN = "2019040002"; // Numero de Serie 10 caracteres 
int DeviceMonth = 4; // Mes de Fabrico
int DeviceYear = 2019; // Ano de Fabrico
String DeviceDeveloper = "Joao Pires"; // Desenvolvido Por
String StrDeviceInfo = "|2019040002|4|2019"; // Enviar Informação do Aparelho para VB --> |Device SN|Device Month|Device Year 
...
int ScreenSaverDelay=5; // Delay de 5 segundos para chamar proteção de Ecra
...
//*************Declaração Sensor Temperatura & Humidade************
  dht DHT;
  #define DHT22_PIN 31 // Declaração do sensor temperatura&Humidade pino 31
...
// Rotina para chamada da leitura de temperatura e humidade
...
// Chamar Rotina para controlo de temperatura em Automatico
    if(MachineStatus==1){AutoTempControl(); AutoHumControl();}
...
// Chamar Rotinas para Registo de Eventos de Temperature e Humidade para Graficos e Tabela
   if((Minute==0 || Minute==15 || Minute==30 || Minute==45) && Second==59 && Flg_TempHumAlarm == false && Flg_RTCAlarm==false){
      ...
      GraphSaveHumidity(); // Chamar Rotina para guardar valor da Humidade para gráfico
      SaveTempEventData(); // Chamada para escrever Registo para Ecra 20 (Tabela de Registos)
      SaveHumEventData(); // Chamada para escrever Registo para Ecra 20 (Tabela de Registos)
      if(CurrentScreen==20){
  ...
  if (boPulse_1s==true){ //***************************** Pulse 1s - Chamada de Rotinas ****************************************************
 ...
 void GetTempHum(){
   ...
   Aux_HumDirect = (((Val_HumDirect*10)+Val_HumDirectAux)/10);
   Aux_HumCalib = (((Val_HumCalib*10)+Val_HumCalibAux)/10);
  
    DHT.read22(DHT22_PIN);
    
    Temperature = ((DHT.temperature* Aux_TempCalib) / Aux_TempDirect);
    if(Temperature>=80){Temperature=80;}
    
    Humidity = ((DHT.humidity* Aux_HumCalib) / Aux_HumDirect);
...
// Button Down
    ...
    else if(CurrentScreen==20 || CurrentScreen==27){
...
// *************************** Função para Touch *****************************
 ...
 if (p.z > MINPRESSURE && p.z < MAXPRESSURE){
   ...
   if(p.x>464 && p.x<551 && p.y>537 && p.y<860 && CurrentScreen==1){ // Botao Manual
    ...
    if(p.x>165 && p.x<275 && p.y>129 && p.y<448 && CurrentScreen==1){ // Clicar na Temperatura em Home Screen
      pinMode(XM, OUTPUT);
      pinMode(YP, OUTPUT);
    ...
    if(p.x>165 && p.x<275 && p.y>539 && p.y<862 && CurrentScreen==1){ // Clicar na Humidade em Home Screen
      pinMode(XM, OUTPUT);
      pinMode(YP, OUTPUT);
      HumHomePressed=true; 
      CurrentScreen=21;
  ...
  if(ButtonUp==true){  
     switch (CurrentScreen) 
      {                   
       ...
       break;
       case 6:
          if(TCalibPressed==true && Val_TempCalib < 80){
             Val_TempCalibAux++;
          ...
          if(TDirectPressed==true && Val_TempDirect < 80){
             Val_TempDirectAux++;
       ...
       break;       
       case 16:
          if(HCalibPressed==true && Val_HumCalib < 99){
             Val_HumCalibAux++;
             if(Val_HumCalibAux>9){
                Val_HumCalib++;
                Val_HumCalibAux=0;
             }
            EEPROM.update(11,Val_HumCalib);
            EEPROM.update(47,Val_HumCalibAux);
            Screen16UpdateHCalib();
          }
          if(HDirectPressed==true && Val_HumDirect < 99){
             Val_HumDirectAux++;
             if(Val_HumDirectAux>9){
                Val_HumDirect++;
                Val_HumDirectAux=0;
             }
            EEPROM.update(10,Val_HumDirect);
            EEPROM.update(46,Val_HumDirectAux);
            Screen16UpdateHDirect();            
          }     
       break;
       case 18:
...
// Linhas para escrever titulos
    ...
    tft.setTextColor(GREENYELLOW);
    tft.setTextSize(2);
    tft.print("Controlo da Temperatura");
...
//Escrever Valor Humidade Calibrada 
 ...
 //Escrever Valor Humidade Directa
    tft.fillRect(8,105,105,65,BLACK);    
    if(Val_HumDirect<10){tft.setCursor(25,125);}else if(Val_HumDirect <100){tft.setCursor(15,125);}else{tft.setCursor(10,125);}
    if(HDirectPressed==true){tft.setTextColor(YELLOW);}
    if(HDirectPressed==false){tft.setTextColor(WHITE);}
    tft.setTextSize(4);
    tft.print(Val_HumDirect); 
    tft.print("."); 
    tft.print(Val_HumDirectAux); 
 }
...
//Escrever Valor Humidade Calibrada 
    tft.fillRect(162,105,105,65,BLACK);
    if(Val_HumCalib<10){tft.setCursor(180,125);}else if(Val_HumCalib<100){tft.setCursor(170,125);}else{tft.setCursor(165,125);}
    if(HCalibPressed==true){tft.setTextColor(YELLOW);}
    if(HCalibPressed==false){tft.setTextColor(WHITE);}
    tft.setTextSize(4);
    tft.print(Val_HumCalib);
    tft.print(".");
    tft.print(Val_HumCalibAux);
 }
 ...
 //Escrever Valor Temperatura Directa
  ...
  void Screen27Update(){
    ...
    if(DisplayOutputsScreen1==0){
        ...
        tft.setTextColor(YELLOW);
        tft.setTextSize(1);
        tft.print("RL1:Temperatura / RL2: Humidade");
     }
    if(DisplayOutputsScreen1==1){
        tft.setTextColor(WHITE);
        tft.print("RL3 & RL4");
...
//******* Estado da Temperatura da Agua
 if(CurrentScreen==5){ 
   if(Temperature<TemperatureMin){tft.fillCircle(75,169,15,RED);}else if (Temperature>=TemperatureMin && Temperature<TemperatureMax){tft.fillCircle(75,169,15,YELLOW);} else if (Temperature >= TemperatureMax){tft.fillCircle(75,169,15,GREEN);}
 }   
}

  void PrintHumidityStatus(){

//******* Estado da Humidade
 if(CurrentScreen==15){ 
   if(Humidity<HumidityMin){tft.fillCircle(75,169,15,RED);}else if (Humidity>=HumidityMin && Humidity<HumidityMax){tft.fillCircle(75,169,15,YELLOW);} else if (Humidity >= HumidityMax){tft.fillCircle(75,169,15,GREEN);}
  }   
}
...
//**** Estado do Contacto Relé de Saída
 ...
 if(CurrentScreen==1){
    ...
    if(DisplayOutputsScreen1==1){
       ...
       if(TemperatureControl== 1 && RL1OutputStatus == 1){tft.fillCircle(300,155,7,GREEN);}
       if(TemperatureControl== 1 && RL1OutputStatus == 0){tft.fillCircle(300,155,7,RED);}
       if(TemperatureControl== 2 && RL1OutputStatus == 1){tft.fillCircle(300,155,7,RED);}
       if(TemperatureControl== 2 && RL1OutputStatus == 0){tft.fillCircle(300,155,7,GREEN);} 
      
       if(HumidityControl== 1 && RL2OutputStatus == 1){tft.fillCircle(300,182,7,GREEN);}
       if(HumidityControl== 1 && RL2OutputStatus == 0){tft.fillCircle(300,182,7,RED);}
       if(HumidityControl== 2 && RL2OutputStatus == 1){tft.fillCircle(300,182,7,RED);}
       if(HumidityControl== 2 && RL2OutputStatus == 0){tft.fillCircle(300,182,7,GREEN);}  
    }        
  } 
         
    if(CurrentScreen==5){   
      RL1OutputStatus = digitalRead(DO_Temperature);
      if(TemperatureControl== 1 && RL1OutputStatus == 1){tft.fillCircle(235,140,15,GREEN);}
      if(TemperatureControl== 1 && RL1OutputStatus == 0){tft.fillCircle(235,140,15,RED);}
      if(TemperatureControl== 2 && RL1OutputStatus == 1){tft.fillCircle(235,140,15,RED);}
      if(TemperatureControl== 2 && RL1OutputStatus == 0){tft.fillCircle(235,140,15,GREEN);}      
    } 
  ...
  void AutoTempControl(){
     ...
     if (Temperature >= TemperatureMax || Flg_TempHumAlarm==true){                         
          if(TemperatureControl==1){digitalWrite(DO_Temperature,LOW);}             
          if(TemperatureControl==2){digitalWrite(DO_Temperature,HIGH);}         
     } 
     if (Temperature <= TemperatureMin && Flg_TempHumAlarm==false){         
...
//************************************************* Ecra Screen Saver  *********************************************   
  ...
  void Screen100(){ // Protecao de Ecra 
    ...
    tft.fillScreen(BLACK);
    tft.drawRect(x-4,y-4,148,68,WHITE);
    tft.drawLine(x-4,y+18,x+142,y+18,WHITE);
    tft.drawLine(x+66,y-4,x+66,y+18,WHITE);
    tft.setTextColor(YELLOW);
    tft.setTextSize(2);
    tft.setCursor(x, y);
    if(Temperature<100 && Flg_TempHumAlarm==false){tft.println(Temperature,1);}
    ...
    tft.setTextSize(1);
    tft.setCursor(x+50, y-2);
    tft.print("o");
    tft.setCursor(x+56, y+3);
    tft.print("C");    
    tft.setTextSize(2);
    tft.setCursor(x+72, y);
    if(Humidity<100 && Flg_TempHumAlarm==false){tft.println(Humidity,1);}
    else if(Humidity>=100 && Flg_TempHumAlarm==false){tft.println(Humidity,0);}
    else if(Flg_TempHumAlarm==true){tft.println("##.#");}
    tft.setTextSize(1);
    tft.setCursor(x+123, y+3);
    tft.print("%RH"); 

   if(Flg_RTCAlarm==false){  
      tft.setTextColor(WHITE);
      tft.setTextSize(2);
      tft.setCursor(x+7, y+25);
      if(Day<10){tft.print("0");
      tft.print(Day);}
      if(Day>=10){tft.print(Day);}
      tft.print("/");
